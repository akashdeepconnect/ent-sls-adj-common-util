...
@patch('lambda_function.get_cross_account_session',mock.MagicMock(return_value=None))
@patch('checks.execute_query_from_athena',mock.MagicMock(return_value = pd.read_csv('Tests/athena_data/data.csv', dtype=str)))
...
def test_lambda_handler_emp_lastname_less_3_char(table):
    ...
    items = get_emp_history_data(table,'6502',logger)
    logger.info(items)
    logger.info(len(checks.execute_query_from_athena.return_value))
    assert items == test_lambda_handler_emp_lastname_less_3_char_result

@patch('lambda_function.get_cross_account_session',mock.MagicMock(return_value=None))
@patch('checks.execute_query_from_athena',mock.MagicMock(return_value = pd.read_csv('Tests/athena_data/empty_data.csv', dtype=str)))
@patch('checks.get_today_loc_dt',mock.MagicMock(return_value=datetime(2021, 5, 18)))
